...
class IssueLearningCredential(
    ...
    companion object {
        ...
        // Piloto UIS: fuente de datos académicos reales de la UIS, en vez de randomLearningCredentials (solo demo). Usa
        // authorizationContext.username -el identificador ya validado del
        // titular autenticado vía Keycloak- para buscar su registro en el
        // padrón de graduados. Si la persona está autenticada pero no tiene
        // título registrado, se eleva el error de negocio ya existente en
        // el sealed interface IssueCredentialError, en vez de devolver
        // datos parciales o inventados.

        fun uisDegreeCredentials(
            padronGraduados: eu.europa.ec.eudi.pidissuer.adapter.out.academicregistry.PadronGraduadosClient,
        ): GetAttestationAttributes<LearningCredential> =
            object : GetAttestationAttributes<LearningCredential> {
                context(
                    _: Raise<IssueCredentialError.AttestationDatasetNotFound>,
                    authorizationContext: AuthorizationContext,
                )
                override suspend fun invoke(): LearningCredential {
                    val registro =
                        padronGraduados.buscarPorUsuario(authorizationContext.username)
                            ?: raise(IssueCredentialError.AttestationDatasetNotFound)

                    return LearningCredential.fromUisAcademicRecord(registro)
                }
            }
    }
}
...
private fun cfg(
    ...
    SdJwtVcCredentialConfiguration(
        ...
        display =
            nonEmptyListOf(
                // Piloto UIS: nombre visible en la Wallet. El vct de abajo
                // NO cambia a propósito (sigue siendo
                // urn:eu.europa.ec.eudi:learning:credential:1) para no
                // requerir cambios en ningún verifier que ya lo reconozca.
                CredentialDisplay(
                    DisplayName.en("UIS - Credencial Académica Digital"),
                ),
            ),
...
fun SdJwtObjectBuilder.learningCredential(learningCredential: LearningCredential) {
    ...
    with(learningCredential) {
        ...
        academicLevel?.let { sdClaim(SdJwtVcClaims.AcademicLevel.name, it.value) }
        academicProgram?.let { sdClaim(SdJwtVcClaims.AcademicProgram.name, it.value) }
        programCode?.let { sdClaim(SdJwtVcClaims.ProgramCode.name, it.value) }
        degreeTitle?.let { sdClaim(SdJwtVcClaims.DegreeTitle.name, it.value) }
        ...
        identificationType?.let {
            sdClaim(SdJwtVcClaims.LearnerIdentificationType.name, it.value)
        }
