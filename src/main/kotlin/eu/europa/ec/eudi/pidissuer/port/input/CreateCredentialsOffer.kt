...
class CreateCredentialsOffer(
    ...
    operator fun invoke(request: Request): Uri =
        context(metadata, defaultCredentialOfferUri, allowedSchemes) {
            val credentialOffer = validate(request.credentialConfigurationIds).authorizationCodeGrantOffer(request.academicProgramId)
    ...
    data class Request(
        ...
        val academicProgramId: Long? = null,
    )
...
private fun NonEmptySet<CredentialConfigurationId>.authorizationCodeGrantOffer(academicProgramId: Long?): CredentialsOfferTO {
    val authorizationCode =
        AuthorizationCodeTO(
            issuerState = academicProgramId?.toString(),
