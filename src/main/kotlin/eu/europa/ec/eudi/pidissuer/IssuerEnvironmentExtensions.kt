...
// Piloto UIS: conexión R2DBC DEDICADA al padrón de graduados (Postgres
// externo, distinto del propio del pid-issuer). Spring Boot autoconfigura
// una única DatabaseClient a partir de SPRING_R2DBC_URL -esa es la de
// issued_credential-, así que esta segunda conexión se arma a mano en vez
// de registrarse como un bean DatabaseClient genérico: con dos
// DatabaseClient en el contexto, bean() (BeanRegistrarDsl) no sabría a cuál
// resolver. Devolver directamente PadronGraduadosClient ya construido evita
// esa ambigüedad por completo.
fun Environment.academicRegistryClient(): eu.europa.ec.eudi.pidissuer.adapter.out.academicregistry.PadronGraduadosClient {
    val host = getRequiredPropertyOrEnvVariable<String>("academicregistry.db.host")
    val port = getProperty<Int>("academicregistry.db.port") ?: 5432
    val database = getRequiredPropertyOrEnvVariable<String>("academicregistry.db.database")
    val username = getRequiredPropertyOrEnvVariable<String>("academicregistry.db.username")
    val password = getRequiredPropertyOrEnvVariable<String>("academicregistry.db.password")
    ...
    return eu.europa.ec.eudi.pidissuer.adapter.out.academicregistry
        .PadronGraduadosClientPostgres(client)
}
    ...
    fun learningCredentialInSdJwtVc(
        ...
        return IssueLearningCredential(
            ...
            getAttestationAttributes =
                IssueLearningCredential.uisDegreeCredentials(padronGraduados),
            issuerSigningKey = issuerSigningKey,
